...
*** Orb [[file:TestMonsterMeld/Assets/Scripts/Orb.cs][Orb.cs]]

**** Private Variables
     | Name     | Description                                            |
     |----------+--------------------------------------------------------|
     | target   | The Monster or Player the orb follows to be collected. |
     | velocity | The orb's velocity                                     |

**** Private Constants
     | Name   | Description                                      |
     |--------+--------------------------------------------------|
     | SPEED  | The speed at which the orb follows its target at |
     | RADIUS | How close the orb needs to be to be 'Collected'  |

*** OrbPouch [[file:TestMonsterMeld/Assets/Scripts/OrbPouch.cs][OrbPouch.cs]]

**** Private Variables
     | Name | Description                                    |
     |------+------------------------------------------------|
     | orbs | The amount of orbs this OrbPouch has collected |
...
**** Public Methods
     | Name     | Description                   | Arguments                         |
     |----------+-------------------------------+-----------------------------------|
     | AddOrb   | Increments 'orbs' by 1        |                                   |
     | AddOrbs  | Increases 'orbs' by an amount | amount: the amount to increase by |
     | TakeOrbs | Removes an amount from 'orbs' | amount: the amount to remove      |

*** OrbFeeder [[file:TestMonsterMeld/Assets/Scripts/OrbFeeder.cs][OrbFeeder.cs]] : OrbPouch [[file:TestMonsterMeld/Assets/Scripts/OrbPouch.cs][OrbPouch.cs]]

**** Public Methods
     | Name   | Description                                              | Arguments |
     |--------+----------------------------------------------------------+-----------|
     | AddOrb | Heals the attached monster instead of collecting the orb |           |
...
** Enemies
  + [-] Circle health bars [3/6]
    + [X] Model
    + [X] Radial Shader
    + [X] Always face the camera
    + [ ] Scale health from center
    + [ ] Clipping Issue
    + [ ] Texture
  + [ ] Low health enemies are less saturated
  + [ ] Battle Camera
  + [ ] Crafted with the fusion box when you put it down
    + Fusion box as respawn point
  + ? Monsters get crafted during battle?
    + Fixes overcrowding
  + Monsters absorb matching element orbs to get health back
    + All monsters eat berries to heal (regardless of type)?
  + Monsters take time to chew orbs
...
** FusionBox
   + [X] Monsters spawn from fusion box
   + [ ] Monsters drop orbs on death
   + [ ] orbs required to 'craft' Monsters
* Pseudo Code
